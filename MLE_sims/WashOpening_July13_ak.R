...
#initcond <- read.csv("/Users/aliciakraay/Dropbox/COVID Shields Emory/Summary_OUT.csv")
#initcond <- read.csv("/Users/aliciakraay/Dropbox/COVID Shields Emory/2020-10-07_wash_chains_summary.csv")
...
p<-initcond$symptomatic_fraction
q<-initcond$q
sd.other<-initcond$socialDistancing_other
preduced<-initcond$p_reduced
c<-initcond$c
...
#Fitted parameters
#Fits<-read.csv('/Users/aliciakraay/Dropbox/COVID Shields Emory/sample.params.csv')
#Fit.sweep<-subset(Fits, Fits$loc=='wash')
#cval2<-0.13
#p=0.48
#q=0.022
#sd.other<-0.17
...
#Set to be November 1, 2020
start.time.test<-c(start.time.reopen, as.numeric(as.Date('11/01/2020', format='%m/%d/%Y')-as.Date('02/13/2020', format='%m/%d/%Y')))
sch.red<-0.5
...
Total.time
beep(sound=3)
save.image('/Users/aliciakraay/Dropbox/COVIDShieldsEmory/MetroFits/Wash_Reopening_July14.RData')

require(ggpubr)

NoImmune_time<-subset(model_time, model_time$specificity==0.5 & model_time$alpha==5 & start.time.test==262 &
...
Wash_sp50<-ggplot(NoImmune_time2, aes(x=time, y=Hospital, group=sweepnum))+geom_line(aes(color=factor(testval)))+
  scale_color_viridis_d(option="plasma", begin = 0.1, end = 0.9, direction = -1,
                        name="Per capita testing rate",
                        labels=c("No testing", "1% per year", "10% per year",  "Yearly", "Monthly"))+
  labs(x='Days since February 13, 2020', y='Hospitalized patients')
ggsave(plot=Wash_sp50, filename='/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/Sp50HospitalAll_Wash.pdf')
...
write.csv(End, '/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/WashReopening_Sweep2_Endvals.csv')
...
End.early<-subset(End, End$start.time.test==108)
alphaplot <- 1
ggplot(data=subset(End, End$alpha==1), 
...
#For Washington, model predicts only ~7% infected after 1 year given ongoing levels of social distancing without
#new case introductions
...
ggplot(data=subset(End, End$alpha==alphaplot), 
  ...
  theme(legend.position=c(0.3, 0.7),
        ...
        axis.line.x = element_line(colour = 'black', size=0.25, linetype='solid'),
        axis.line.y = element_line(colour = 'black', size=0.25, linetype='solid'),
        axis.ticks.length=unit(.25, "cm"))+labs(color='Specificity', linetype='Start testing') -> fig1a5; fig1a5
...
alphaplot<-5
subset(model_time2 , model_time2$alpha == alphaplot &
         model_time2$specificity %in% c(0.96, 0.998) &
         model_time2$testval %in% c(0, N/(365*100), N/(365*10), N/365, N/30) &
         model_time2$start.time.test==262) -> model_time_plot
model_time_plot %>%
  ...
  ggtitle("Washington, 5:1 Shielding") +
  theme(#legend.position='none',
    ...
    axis.line.x = element_line(colour = 'black', size=0.25, linetype='solid'),
    axis.line.y = element_line(colour = 'black', size=0.25, linetype='solid'),
    axis.ticks.length=unit(.25, "cm"))+labs(color='Per capita testing rate', linetype='Specificity') -> fig2a5; fig2a5

subset(model_time2 , model_time2$alpha == alphaplot &
         model_time2$specificity %in% c(0.96, 0.998) &
         model_time2$testval %in% c(0, N/(365*100), N/(365*10), N/365, N/30) &
         model_time2$start.time.test==108) -> model_time_plot
model_time_plot %>%
  ggplot(aes(x=time, y=Released/N, group = sweepnum, color = factor(testval))) +
  geom_line(aes(linetype=factor(specificity))) +
  xlab("Days since Jan 15, 2020") +
...
model_time_plot %>%
  ...
  #labs(color = "Per capita testing rate") +
  geom_vline(xintercept = dates[c(1,3,4)], color = "grey60") +
  annotate("text", x = dates[1], y=40, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[3], y=40, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[4]+10, y=40, label=paste("\nOpen schools"), angle=90, color="grey40", size=4) +
  ggtitle("Washington, 5:1 Shielding") +
  theme(#legend.position='none',
    legend.position=c(0.4, 0.7),    
    legend.key = element_blank(),
    ...
    axis.line.x = element_line(colour = 'black', size=0.25, linetype='solid'),
    axis.line.y = element_line(colour = 'black', size=0.25, linetype='solid'),
    axis.ticks.length=unit(.25, "cm"))+labs(linetype='Specificity') -> fig2a1; fig2a1
