#General setup
#This script estimates impact of a shielding strategy for S Florida
...
seir_model_shields_nopreH <- function(t, x, params) {
  ...
  WorkContacts_10x10_Distancing2[5,]<-c(WorkContacts_5x5[3,1]*FracReleased[1]*preduced2, 
                                        WorkContacts_5x5[3,1]*FracDistanced[1]*preduced2, 
...
#Initial conditions are from Conan - absolute file path for K's PC
initcond<- read.csv("/Users/aliciakraay/Dropbox/COVID Shields Emory/2020-10-07_sflor_chains_summary.csv")
initcond10<-initcond[c(2:11),]
means<-apply(initcond10, 2, mean)
# base model
start = c(S.c = as.numeric(means["S_c"]), E.c =as.numeric(means["E_c"]), Is.c = as.numeric(means["Isym_c"]), Ia.c = as.numeric(means["Iasym_c"]), 
          Hs.c = as.numeric(means["Hsub_c"]), Hc.c =as.numeric(means["Hcri_c"]), D.c = as.numeric(means["D_c"]), R.c = as.numeric(means["R_c"]), 
          S.c.pos = 0, E.c.pos = 0, Is.c.pos = 0, Ia.c.pos = 0, R.c.pos =0, 
          
          S.a = as.numeric(means["S_a"]), E.a = as.numeric(means["E_a"]), Is.a = as.numeric(means["Isym_a"]), Ia.a = as.numeric(means["Iasym_a"]), 
          Hs.a = as.numeric(means["Hsub_a"]), Hc.a = as.numeric(means["Hcri_a"]), D.a = as.numeric(means["D_a"]), R.a = as.numeric(means["R_a"]), 
          S.a.pos = 0, E.a.pos = 0, Is.a.pos = 0, Ia.a.pos = 0, R.a.pos =0,
          
          S.rc = as.numeric(means["S_rc"]), E.rc = as.numeric(means["E_rc"]), Is.rc = as.numeric(means["Isym_rc"]), Ia.rc = as.numeric(means["Iasym_rc"]), 
          Hs.rc = as.numeric(means["Hsub_rc"]), Hc.rc = as.numeric(means["Hcri_rc"]), D.rc = as.numeric(means["D_rc"]), R.rc = as.numeric(means["R_rc"]), 
          S.rc.pos = 0, E.rc.pos = 0, Is.rc.pos = 0, Ia.rc.pos = 0, R.rc.pos =0,
          
          S.fc = as.numeric(means["S_fc"]), E.fc = as.numeric(means["E_fc"]), Is.fc = as.numeric(means["Isym_fc"]), Ia.fc = as.numeric(means["Iasym_fc"]), 
          Hs.fc = as.numeric(means["Hsub_fc"]), Hc.fc = as.numeric(means["Hcri_fc"]), D.fc = as.numeric(means["D_fc"]), R.fc = as.numeric(means["R_fc"]), 
          S.fc.pos = 0, E.fc.pos = 0, Is.fc.pos = 0, Ia.fc.pos = 0, R.fc.pos =0,
          
          S.e = as.numeric(means["S_e"]), E.e = as.numeric(means["E_e"]), Is.e = as.numeric(means["Isym_e"]), Ia.e = as.numeric(means["Iasym_e"]), 
          Hs.e = as.numeric(means["Hsub_e"]), Hc.e= as.numeric(means["Hcri_e"]), D.e = as.numeric(means["D_e"]), R.e = as.numeric(means["R_e"]), 
          S.e.pos = 0, E.e.pos = 0, Is.e.pos = 0, Ia.e.pos = 0, R.e.pos =0,
          H.pos = 0)

p<-as.numeric(means['symptomatic_fraction'])
q<-as.numeric(means['q'])
sd.other<-as.numeric(means['socialDistancing_other'])
preduced<-as.numeric(means['p_reduced'])
c<-as.numeric(means['c'])
...
#Intervention timing based on local policies, S. Florida here
start.time.distance <- as.numeric(c(as.Date('03/17/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')))
start.time.reopen <- as.numeric(c(as.Date('05/20/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')))
school.start.time <- as.numeric(c(as.Date('9/01/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')))
...
Total.time
save.image('/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/Sflorida_Reopening_SweepLong.RData')
#save.image('/Users/kristinnelson/Box Sync/covid-19/Shielding/Results/Sflorida_Reopening_Sweep2.RData')

#beep(sound=3)
require(ggpubr)

NoImmune_time<-subset(model_time, model_time$specificity==0.5 & model_time$alpha==5 & start.time.test==248 &
                        model_time$testval %in% c(N/(365*100), N/(365*10), N/365, max(model_time$testval)))
NoTest<-subset(model_time, model_time$testval==0)

NoImmune_time2<-rbind(NoImmune_time, NoTest)

ggplot(NoImmune_time2, aes(x=time, y=Hospital.pos/Released, group=sweepnum))+geom_line(aes(color=factor(testval)))+
  ...
  labs(x='Days since February 20, 2020', y='Fraction of test-positive \n individuals hospitalized')

Sfl_sp50<-ggplot(NoImmune_time2, aes(x=time, y=Hospital, group=sweepnum))+geom_line(aes(color=factor(testval)))+
  scale_color_viridis_d(option="plasma", begin = 0.1, end = 0.9, direction = -1,
                        name="Per capita testing rate",
                        labels=c("No testing", "1% per year", "10% per year",  "Yearly", "Monthly"))+
  labs(x='Days since February 27, 2020', y='Hospitalized patients')
ggsave(plot=Sfl_sp50, filename='/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/Sp50HospitalAll_Sfl.pdf')
...
###First plot set: deaths###
dates <- c(distancetime, testtime, reopentime, schooltime)
End<-subset(model_time, model_time$time==365)
#write.csv(End, '/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/SFlaReopening_LateStart.csv')
...
alphaplot <- 5
ggplot(data=subset(End, End$alpha==alphaplot), 
       aes(x=testval, y=Deaths, group=interaction(start.time.test, specificity)))+
  geom_line(aes(linetype=factor(start.time.test), color=factor(specificity)))+
  scale_linetype_manual(values=c("dotted", "solid")) +
  scale_color_viridis_d(begin = 0, end = 0.9) +
  xlab("Daily testing rate") +
...
model_time_plot2 %>%
  ...
  ggtitle("S Florida, 5:1 Shielding") +
  theme(legend.position='none',
...
model_time_plot2<-subset(model_time, model_time$alpha == alphaplot & model_time$specificity %in% c(0.96, 0.998) & 
                           model_time$testval %in% c(0, N/(365*100), N/(365*10), N/(365), N/30) &
                           model_time$start.time.test == as.numeric(as.Date('11/01/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')))
...
###Third plot set: critical care cases
model_time_plot2 %>%
  ...
  #labs(color = "Per capita testing rate") +
  geom_vline(xintercept = dates[c(1,3,4)], color = "grey60") +
  annotate("text", x = dates[1], y=450, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[3], y=450, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[4]+10, y=450, label=paste("\nOpen schools"), angle=90, color="grey40", size=4) +
  ggtitle("S Florida, 5:1 Shielding") +
  theme(#legend.position='none',
    legend.position=c(0.4, 0.7),    
    legend.key = element_blank(),
    ...
    axis.line.y = element_line(colour = 'black', size=0.25, linetype='solid'),
    axis.ticks.length=unit(.25, "cm"))+
  labs(color='Per capita testing rate', linetype='Specificity') -> fig3a5; fig3a5

##Fourth plot set: cumulative hospitalizations among seropositive individuals
model_time_plot %>%
  ggplot(aes(x=time, y=Hospital.pos, group = sweepnum, color = factor(testval))) +
  geom_line(aes(linetype=factor(specificity))) +
  xlab("Days since Jan 15, 2020") +
  ylab("Cumulative hospitalizations among seropositive individuals") +
  scale_color_viridis_d(option="plasma", begin = 0.1, end = 0.9, direction = -1,
                        name="Per capita testing rate",
                        labels=c("No testing", "1% per year", "10% per year",  "Yearly", "Monthly")) +
  #labs(color = "Per capita testing rate") +
  geom_vline(xintercept = dates[c(1,3,4)], color = "grey60") +
  annotate("text", x = dates[1], y=200, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[3], y=200, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[4]+10, y=200, label=paste("\nOpen schools if \n seropositives > 10%"), angle=90, color="grey40", size=4) +
  ggtitle("S Florida, 5:1 Shielding") +
  theme(#legend.position='none',
    legend.position=c(0.4, 0.7),    
    legend.key = element_blank(),
    ...
    axis.line.y = element_line(colour = 'black', size=0.25, linetype='solid'),
    axis.ticks.length=unit(.25, "cm"))+
  labs(color='Per capita testing rate', linetype='Specificity')-> fig4a5; fig4a5
...
Spec96<-subset(End, End$specificity==0.96)
fig <- plot_ly(
  y = Spec96$Released, 
  x = 1-Spec96$cval, 
  z = Spec96$Deaths, 
...
Spec100<-subset(SD_test_sweep3, SD_test_sweep$specificity==1.00)
fig100 <- plot_ly(
  y = Spec100$Released, 
  x = 1-Spec100$cval, 
  z = Spec100$Deaths, 
...
ggplot(data=SD_test_sweep, aes(x=testval, y=Deaths, group = factor(cval), color = factor(cval))) +
  geom_line() +
  scale_color_viridis(discrete=TRUE) +
  xlab("Daily testing rate") +
  ylab("Cumulative deaths") +
  labs(color = "Distancing intensity") +
  ...
  # Excel results
subset(End, End$testval==0)
subset(End, End$alpha==5 & End$specificity==0.998 & End$start.time.test==248 & End$testval==N/30)
subset(End, End$alpha==5 & End$specificity==0.998 & End$start.time.test==248 & End$testval==N/365)
subset(End, End$alpha==5 & End$specificity==0.998 & End$start.time.test==83 & End$testval==N/30)
subset(End, End$alpha==5 & End$specificity==0.998 & End$start.time.test==83 & End$testval==N/365)
subset(End, End$alpha==5 & End$specificity==0.5 & End$start.time.test==248 & End$testval==N/30)
subset(End, End$alpha==1 & End$specificity==0.96 & End$start.time.test==248 & End$testval==N/30)
subset(End, End$alpha==5 & End$specificity==0.96 & End$start.time.test==248 & End$testval==N/30)
