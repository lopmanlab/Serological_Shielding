...
seir_model_shields_nopreH <- function(t, x, params) {
  ...
  #Work contacts with targeted distancing and new relaxation/lockdown
  #DISTANCING CONDITIONAL ON TESTING: People working from home regain their workplace contacts if they test positive 
  #People in reduced and full contact occupations have the distribution of their workplace contacts changed by alpha
  WorkContacts_10x10_TargetedDistancing2<-matrix(0, nrow=10, ncol=10)
  WorkContacts_10x10_TargetedDistancing2[3,]<-c(WorkContacts_5x5[2,1], 0, WorkContacts_5x5[2,2], 0, WorkContacts_5x5[2,3], 0,
                                                WorkContacts_5x5[2,4], 0, WorkContacts_5x5[2,5], 0)
  ...
  WorkContacts_10x10_TargetedDistancing2[1,]<-c(rep(0, 10))
  DistancedWork_RedContact2<-WorkContacts_10x10_Baseline[5,]*preduced3
  WorkContacts_10x10_TargetedDistancing2[5,1]<-sum(DistancedWork_RedContact2[1], DistancedWork_RedContact2[2])*Scale[1]
  WorkContacts_10x10_TargetedDistancing2[5,2]<-sum(DistancedWork_RedContact2[1], DistancedWork_RedContact2[2])*(1-Scale[1])
  WorkContacts_10x10_TargetedDistancing2[5,3]<-sum(DistancedWork_RedContact2[3], DistancedWork_RedContact2[4])*Scale[2]
  WorkContacts_10x10_TargetedDistancing2[5,4]<-sum(DistancedWork_RedContact2[3], DistancedWork_RedContact2[4])*(1-Scale[2])
  WorkContacts_10x10_TargetedDistancing2[5,5]<-sum(DistancedWork_RedContact2[5], DistancedWork_RedContact2[6])*Scale[3]
  WorkContacts_10x10_TargetedDistancing2[5,6]<-sum(DistancedWork_RedContact2[5], DistancedWork_RedContact2[6])*(1-Scale[3])
  WorkContacts_10x10_TargetedDistancing2[5,7]<-sum(DistancedWork_RedContact2[7], DistancedWork_RedContact2[8])*Scale[4]
  WorkContacts_10x10_TargetedDistancing2[5,8]<-sum(DistancedWork_RedContact2[7], DistancedWork_RedContact2[8])*(1-Scale[4])
  WorkContacts_10x10_TargetedDistancing2[5,9]<-sum(DistancedWork_RedContact2[9], DistancedWork_RedContact2[10])*Scale[5]
  WorkContacts_10x10_TargetedDistancing2[5,10]<-sum(DistancedWork_RedContact2[9], DistancedWork_RedContact2[10])*(1-Scale[5])
  WorkContacts_10x10_TargetedDistancing2[6,]<-WorkContacts_10x10_TargetedDistancing2[5,]
  WorkContacts_10x10_TargetedDistancing2[7,]<-WorkContacts_10x10_TargetedDistancing2[5,]/preduced3
  WorkContacts_10x10_TargetedDistancing2[8,]<-WorkContacts_10x10_TargetedDistancing2[5,]/preduced3
  ...
  #Fix other contacts with a new value of sd.other
  OtherContacts_10x10_TargetedDistancing3<-matrix(0, nrow=10, ncol=10)
  ...
  for(i in 1:5){
    for(j in 1:5){
      ...
      OddVec<-c(1,3,5,7,9)
      #Odd rows
      OtherContacts_10x10_TargetedDistancing3[2*i-1,OddVec[j]]<-OtherContacts_5x5[i,j]*sd.other3*Scale[j]
      OtherContacts_10x10_TargetedDistancing3[2*i-1,EvenVec[j]]<-OtherContacts_5x5[i,j]*sd.other3*(1-Scale[j])
      #Even rows
      OtherContacts_10x10_TargetedDistancing3[2*i,OddVec[j]]<-OtherContacts_5x5[i,j]*sd.other3*Scale[j]
      OtherContacts_10x10_TargetedDistancing3[2*i,EvenVec[j]]<-OtherContacts_5x5[i,j]*sd.other3*(1-Scale[j])
    }
  }
  
  OtherContacts_10x10_TargetedDistancing4<-OtherContacts_10x10_TargetedDistancing3
  OtherContacts_10x10_TargetedDistancing4[1,]<-OtherContacts_10x10_TargetedDistancing3[1,]*(1/sd.other3)
  OtherContacts_10x10_TargetedDistancing4[3,]<-OtherContacts_10x10_TargetedDistancing3[3,]*(1/sd.other3)
  OtherContacts_10x10_TargetedDistancing4[5,]<-OtherContacts_10x10_TargetedDistancing3[5,]*(1/sd.other3)
  OtherContacts_10x10_TargetedDistancing4[7,]<-OtherContacts_10x10_TargetedDistancing3[7,]*(1/sd.other3)
  OtherContacts_10x10_TargetedDistancing4[9,]<-OtherContacts_10x10_TargetedDistancing3[9,]*(1/sd.other3)
  ...
  if(t>start.time.reopen & t<school.start.time){ 
    ...
    #Use these matrices when distancing has been relaxed but testing has not started and schools are not open
    
    CM<-HomeContacts_10x10 + WorkContacts_10x10_TargetedDistancing + 
      OtherContacts_10x10_TargetedDistancing2}
  
  if(t>=school.start.time & t<start.time.test){ 
  ...
  if(t>start.time.test){ 
    #Use these matrices with targeting and schools reopening
    CM<-HomeContacts_10x10 + WorkContacts_10x10_TargetedDistancing2 + 
      OtherContacts_10x10_TargetedDistancing4+(sch.red*SchoolContacts_10x10_Baseline)
  }
  ...
  list(res)
  #print(c(tot.c, tot.c.pos, tot.c.gen))
}

#Model parameters

# Population age structure - US population 
...
# New - based on average across fits from three metro locations
...
#Intervention timing based on average of metro area policy start dates 
...
#Varied parameters
dval2<-c(0, 0.25, 0.5, 0.75, 1.0)
alpha2<-c(1, 5)
specificity<-c(0.96, 0.998)
testing<-c(0, N/(365*100), N/(365*10), N/(365*5), N/365, N/180, N/90, N/60, N/30) # scaled to avg pop size in three metro areas
#Set to be November 1, 2020 or reopening time
start.time.test <-as.numeric(as.Date('11/01/2020', format='%m/%d/%Y')-as.Date('02/20/2020', format='%m/%d/%Y'))
sch.red<-0.5

sweep<-expand.grid(dval=dval2, testval=testing, alpha=alpha2,
...
for(i in 1:dim(sweep)[1]){
  ...
  sd.other3<-1-((1-sd.other)*d)
  preduced3<-1-((1-preduced)*d)
...
model_time<-merge(sweep, UnlistDat, by='sweepnum', all.y=TRUE)
...
save.image('/Users/aliciakraay/Dropbox/COVIDShieldsEmory/Contour plots/NYCcontour_Sweep_LockRelax_July9.RData')

load("~/Dropbox/COVIDShieldsEmory/Contour plots/NYCcontour_Sweep_LockRelax_July9.RData")
Nov1<-subset(model_time, model_time$time==testtime)
#m <- list(
#  l = 1,
#  r = 1,
#  b = 1,
...
Nov1<-Nov1[,c(1, 10)]
names(Nov1)[2]<-'Deaths.Nov1'
End2<-merge(Nov1, End, by='sweepnum')
End2$Deaths.shields<-End2$Deaths-End2$Deaths.Nov1
Spec99<-subset(End2, End2$specificity==0.998 & 
                    start.time.test == testtime &
                    !(testval %in% c(N/90, N/60, N/180, N/(365*5))) &
                    alpha==5)

Spec96<-subset(End2, End2$specificity==0.96 & 
                 start.time.test == testtime &
                 !(testval %in% c(N/90, N/60, N/180, N/(365*5))) &
                 alpha==5)
...
#Deaths
figD<-plot_ly(y = Spec99$testval, x = 1-Spec99$dval, z = Spec99$Deaths.shields, type = "contour", 
              colorscale = list(c(0, 0.5, 1), c('blue', 'MediumAquaMarine', 'yellow')), width=600, height=600) 
figD <- figD %>% layout(xaxis=list(title="",
                                   ticktext=list('', '', ''),
                                   tickvals=list(0, 0.5, 1),
                                   tickangle=0,
                                   tickfont=list(size=11)), 
                        yaxis=list(title="Testing", 
                                   ticktext=list('No testing', '1%/yr', '10%/yr', 'Yearly', 'Monthly'),
                                   tickvals=list(0, N/(365*100), N/(365*10), N/365, N/30),
                                   type='log'))
figD
#figD.nyc<-figD %>%   colorbar(title='Cumulative \ndeaths \nNov1-June1')
#figD.nyc


#Released
figR<-plot_ly(y = Spec99$testval, x = 1-Spec99$dval, z = Spec99$Released, type = "contour", 
              colorscale = list(c(0, 0.5, 1), c('yellow', 'MediumAquaMarine', 'blue')), width=600, height=600) 
figR <- figR %>% layout(xaxis=list(title="",
                                  ticktext=list('', '', ''),
                                  tickvals=list(0, 0.5, 1),
                                  tickangle=0,
                                  tickfont=list(size=11)), 
                                  ...
                                  tickvals=list(0, N/(365*100), N/(365*10), N/365, N/30),
                                  type='log'))
#figR <- figR %>% layout(xaxis=list(title="Social contacts level \n Nov 2020-June 2021",
#                                   ticktext=list('lockdown \n(Apr 2020)', '50% \nreduction', 'pre-\npandemic'),
#                                   tickvals=list(0, 0.5, 1),
#                                   tickangle=0,
#                                   tickfont=list(size=11)), 
#                        yaxis=list(title="Testing", 
...
#                                   type='log'))
figR
#figR.nyc<-figR %>%   colorbar(title='Number \nreleased')
#figR.nyc
#orca(figR, '/Users/aliciakraay/Dropbox/COVID Shields Emory/Contour plots/Resize_NYReleased.png')
