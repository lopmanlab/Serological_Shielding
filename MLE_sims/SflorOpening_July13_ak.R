...
#as.numeric(as.Date('06/01/2021', format='%m/%d/%Y')-as.Date('02/20/2020', format='%m/%d/%Y'))
t = (0:474)
#t = (0:364)
...
#here we have 1) early (with SAH order) and 2) late options (11/01/2020)
start.time.test<-c(start.time.reopen, as.numeric(as.Date('11/01/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')))
sch.red<-0.5
...
for(i in 1:dim(sweep)[1]){
  ...
  if(daily.tests==0){
    tswitch1.dat<-data.frame(times=times, test.switch1=c(rep(1, length(t))))
    tswitch2.dat<-data.frame(times=times, test.switch2=c(rep(0, length(t))))
  }
  if(daily.tests>0){
  ...
  params = c('agestruc'=agestruc, 
             ...
             'sensitivity'=sensitivity, 'specificity'=specificity, 'sch.red'=sch.red) 
...
NoImmune_time<-subset(model_time, model_time$specificity==0.5 & model_time$alpha==5 & start.time.test==255 &
...
#NY_sp50<-ggplot(NoImmune_time2, aes(x=time, y=CriticalCare, group=sweepnum))+geom_line(aes(color=factor(testval)))+
#  scale_color_viridis_d(option="plasma", begin = 0.1, end = 0.9, direction = -1,
#                        name="Per capita testing rate",
#                        labels=c("No testing", "1% per year", "10% per year",  "Yearly", "Monthly"))+
#  labs(x='Days since February 20, 2020', y='Hospitalized patients')
#ggsave(plot=NY_sp50, filename='/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/Sp50HospitalAll_NYC.pdf')
end.time<-Sys.time()
Total.time<-end.time-start.time
Total.time
beep(sound=3)
save.image('/Users/aliciakraay/Dropbox/COVIDShieldsEmory/MetroFits/Sflor_Reopening_July14.RData')
require(ggpubr)
...
NoImmune<-subset(End, End$specificity==0.5)
#write.csv(End, '/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/WashReopening_EarlyStart.csv')
...
End2<-subset(End, !(End$testval %in% c(N/90)))
alphaplot <- 5
ggplot(data=subset(End2, End2$alpha==alphaplot), 
       aes(x=testval, y=Deaths, group=interaction(start.time.test, specificity)))+
  #aes(x=testval, y=Deaths, group = factor(cval), color = factor(cval))) +
  geom_line(aes(linetype=factor(start.time.test), color=factor(specificity)))+
  #geom_line(data=subset(SD_test_sweep2, SD_test_sweep2$specificity==1.00 & SD_test_sweep2$alpha==alphaplot), aes(x=testval, y=Deaths, group=factor(cval),
...
subset(model_time , model_time$alpha == alphaplot &
         model_time$start.time.test==255 & model_time$specificity %in% c(0.96, 0.998)) -> model_time_plot
model_time_plot %>%
...
model_time_plot2<-subset(model_time_plot, model_time_plot$testval %in% c(N/(365*100), N/(365*10), N/365, N/30))
model_time_plot3<-subset(model_time, model_time$testval==0)
model_time_plot4<-rbind(model_time_plot2, model_time_plot3)
