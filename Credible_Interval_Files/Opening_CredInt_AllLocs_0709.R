#General setup
#This script estimates impact of a shielding strategy
...
seir_model_shields_nopreH <- function(t, x, params) {
  ...
  child.tests<-daily.tests*agestruc.c
  adult.tests.h<-daily.tests*agestruc.a*prob.home
  adult.tests.rc<-daily.tests*agestruc.a*prob.reduced
  adult.tests.fc<-daily.tests*agestruc.a*prob.full
  el.tests<-daily.tests*agestruc.e
...
#######Model parameters########## 
#####all locations and interventions######
#as.numeric(as.Date('06/01/2021', format='%m/%d/%Y')-as.Date('02/20/2020', format='%m/%d/%Y'))
#as.numeric(as.Date('06/01/2021', format='%m/%d/%Y')-as.Date('02/13/2020', format='%m/%d/%Y'))

t = (0:474)  #Change this for time series length
...
####Location specific parameters, all interventions by site####
#Order is: nyc, sfl, wash
N=c(9437195, 6168615, 4064835)
time.distance<-c(26, 19, 32)
time.reopen<-c(109, 83, 108)
school.start<-c(194, 187, 231)

agestruc.c<-c(0.23, 0.23, 0.24)
agestruc.a<-c(0.63, 0.60, 0.63)
agestruc.e<-c(0.14, 0.18, 0.14)
location.fixed<-data.frame(N=N, start.time.distance=time.distance, start.time.reopen=time.reopen, 
                           school.start.time=school.start, loc=c('nyc', 'sfl', 'wash'), agestruc.c=agestruc.c,
                           agestruc.a=agestruc.a, agestruc.e=agestruc.e, nov1.numeric=c(255, 248, 262))

#####Location specific parameters###
params<-read.csv('/Users/kristinnelson/OneDrive - Emory University/covid-19/Shielding/MCMC Results/samp.params.0622.csv')
params.sweep<-merge(params, location.fixed, by='loc')

#params.sweep2<-subset(params.sweep, params.sweep$loc %in% c('nyc', 'sfl')|(params.sweep$loc=='wash' & 
#                                                                             !(params.sweep$chain=='4\\')))

#params.sweep<-params.sweep2

##########Varied parameters###########
interventions.sweep<-data.frame(testval=c(0, 30, 30, 30, 30, 365, 30, 365), 
                        specificity=c(0.5, 0.5, 0.90, 0.90, 0.998, 0.998, 0.998, 0.998),
                        #start.time.test=c(255, 255, 255, 255, 255, 255, 108, 108),
                        test.timing=c('Nov1', 'Nov1', 'Nov1', 'Nov1', 'Nov1', 'Nov1', 'SAH', 'SAH'),
                        alpha=c(5, 5, 1, 5, 5, 5, 5, 5), 
                        description=c('No testing', 
                                      'Antibodies cant distinguish, monthly testing, with shielding', 
                                      'Suboptimal test (90%), monthly testing, no shielding',
                                      'Suboptimal test (90%), monthly testing, with shielding',
                                      'Ideal test, monthly testing, with shielding', 
                                      'Ideal test, annual testing, with shielding', 
                                      'Early testing, ideal test, monthly testing, with shielding',
                                      'Early testing, ideal test, annual testing, with shielding'))


data.summary<-list(dim(interventions.sweep)[1]*nrow(params.sweep))
data.full<-list(dim(interventions.sweep)[1]*nrow(params.sweep))

for(i in 1:(dim(interventions.sweep)[1]*nrow(params.sweep))){
  data.summary[[i]]<-data.frame()
  data.full[[i]]<-data.frame()
}

start.time<-Sys.time()
for(k in 1:nrow(params.sweep)){
#for(k in 1:2){
    #for(k in 199:202){
    N<-params.sweep$N[k] 
    c<-params.sweep$c[k]
    q<-params.sweep$q[k]
    p<-params.sweep$symptomatic_fraction[k]
    start.time.distance<-params.sweep$start.time.distance[k]
    start.time.reopen<-params.sweep$start.time.reopen[k]
    nov1.numeric<-params.sweep$nov1.numeric[k]
    school.start.time<-params.sweep$school.start.time[k]
    sd.other<-params.sweep$socialDistancing_other[k]
    preduced<-params.sweep$p_reduced[k]
    agestruc.c<-params.sweep$agestruc.c[k]
    agestruc.a<-params.sweep$agestruc.a[k]
    agestruc.e<-params.sweep$agestruc.e[k]
    start<-params.sweep[k,c(12:77)]
    for(l in 1:dim(interventions.sweep)[1]){
    if(interventions.sweep$test.timing[l]=='SAH'){start.time.test<-start.time.reopen}
    if(interventions.sweep$test.timing[l]=='Nov1'){start.time.test<-nov1.numeric}
    if(interventions.sweep$testval[l]==0){daily.tests<-0}
    if(interventions.sweep$testval[l]>0){daily.tests<-N/interventions.sweep$testval[l]}
    alpha<-interventions.sweep$alpha[l]
    specificity<-interventions.sweep$specificity[l]
    ...
    #system.time(ode(y = as.numeric(start), times = t, fun = seir_model_shields_nopreH, parms = par, 
    #    method='ode45'))
    
    model_out = as.data.frame(ode(y = as.numeric(start), times = t, fun = seir_model_shields_nopreH, parms = params, 
                                  method='ode45'))
    names(model_out)<-c('time', names(start))
    data.summary[[(dim(interventions.sweep)[1]*(k-1))+l]]<-ddply(model_out, .(time), summarize, 
                             ...
                             Incidence=N-sum(S.c, S.c.pos, S.a, S.a.pos, S.rc, S.rc.pos, S.fc, S.fc.pos,
                                             S.e, S.e.pos),
                             CI=Incidence/N,
                             TestStart=start.time.test)
    data.full[[(dim(interventions.sweep)[1]*(k-1))+l]]<-model_out
  }  
}


interventions.sweep<-as.data.frame(interventions.sweep)

interventions.sweep$intervention.id<-c(1:nrow(interventions.sweep))

#id.check<-numeric(nrow(interventions.sweep)*nrow(params.sweep))
#check2<-numeric(seq(from=1, to=length(id.check), by=1))
#for(i in 1:length(id.check)){
#  id.check[i]<-(as.numeric(i)/nrow(interventions.sweep)-floor(as.numeric(i)/nrow(interventions.sweep)))
#}
#Got stuck on ideal test, monthly testing, start late, with shielding--l=5, k=96

for(i in 1:(nrow(interventions.sweep)*(nrow(params.sweep)))){
#for(i in 1:16){
  data.summary[[i]]$intervention.id<-(as.numeric(i)/nrow(interventions.sweep)-floor(as.numeric(i)/nrow(interventions.sweep))) #Relative chain position for intervention
  data.full[[i]]$intervention.id<-(as.numeric(i)/nrow(interventions.sweep)-floor(as.numeric(i)/nrow(interventions.sweep))) #Relative chain position for intervention
  data.summary[[i]]$param.id<-floor(as.numeric(i-1)/nrow(interventions.sweep))+1 #Relative chain position for parameters
  data.full[[i]]$param.id<-floor(as.numeric(i-1)/nrow(interventions.sweep))+1 #Relative chain position for parameters
  data.summary[[i]]$runid<-i #Absolute chain position
  data.full[[i]]$runid<-i #Absolute chain position
}
...
UnlistDat<-bind_rows(data.summary)
interventions.sweep$sweepid<-as.numeric(interventions.sweep$intervention.id)/nrow(interventions.sweep)
interventions.sweep$intervention.id<-interventions.sweep$sweepid-floor(interventions.sweep$sweepid)
#UnlistDat$sweepnum<-UnlistDat$sweepid2
model_time<-merge(interventions.sweep[,c(1:6)], UnlistDat, by='intervention.id', all.y=TRUE)
params.sweep$param.id<-seq(from=1, to=nrow(params.sweep), by=1)
model_time2<-merge(model_time, params.sweep, by='param.id')

end.time<-Sys.time()
Total.time<-end.time-start.time
Total.time
End<-subset(model_time, model_time$time==474) 

beep(sound=3)
save.image('/Users/kristinnelson/OneDrive - Emory University/covid-19/Shielding/Results/Opening_CredInt_AllLocs_0714.RData')

#readRDS('/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/Opening_CredInt_AllLocs_0622.RData')
#require(ggpubr)
#End2<-End[c(1:760),]
#write.csv(End, '/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/EndVals_all_locs_CIs.csv')
write.csv(End, '/Users/kristinnelson/OneDrive - Emory University/covid-19/Shielding/Results/EndVals_all_locs_CIs_0714.csv')

CredInt.file<-ddply(End, .(intervention.id, TestStart), summarize, 
                    Death.LCL=quantile(Deaths, probs=0.025), Death.UCL=quantile(Deaths, probs=0.975), 
                    Released.LCL=quantile(FracReleased, probs=0.025), Released.UCL=quantile(FracReleased, probs=0.975), 
                    ICU.LCL=quantile(CriticalCare, probs=0.025), ICU.UCL=quantile(CriticalCare, probs=0.975),
                    CI.LCL=quantile(CI, probs=0.025), CI.UCL=quantile(CI, probs=0.975))
#write.csv(CredInt.file, '/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/CredInts.csv')

write.csv(CredInt.file, '/Users/kristinnelson/OneDrive - Emory University/covid-19/Shielding/Results/CredInts_0714.csv')

#End2<-End[c(1:760),]

#End1<-read.csv('/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/NYCEndVals_sets1to95.csv')
#End2<-read.csv('/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/NYCEndVals_sets96to100.csv')
#End<-rbind(End1, End2)
#table(End$intervention.id)
#write.csv(End, '/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/NYCEndVals_sets1to100.csv')
#Checking completeness
#length(unique(End$runid)) #Only 796 unique runids
#table(End$intervention.id) #These 4 seem to be for intervention row 7, there are also 4 extras
#CheckMissing<-unique(End$runid)
#Possible<-seq(from=1, to=800, by=1)
#Possible[which(!(Possible %in% CheckMissing))]