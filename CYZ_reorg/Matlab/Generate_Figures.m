...
for PARAMETER_SET = ["LANCET"]%["LANCET", "PNAS"]    
    for REGION = ["nyc"]%"sflor", "nyc", "wash"]        
        ...
        saveas(t2, strcat('OUTPUT/', REGION, '/', REGION, '_', PARAMETER_SET, '_', LIKELIHOOD_TYPE, '_chainpanel.png'));

        t3 = figure(3); clf;
        mcmcplot(chain1,[],res1,'pairs');
        saveas(t3, strcat('OUTPUT/', REGION, '/', REGION, '_',  PARAMETER_SET, '_', LIKELIHOOD_TYPE, '_pairs.png'));

        t4 = figure(4); clf;
        mcmcplot(chain1,[],res1,'denspanel',2);
        saveas(t4, strcat('OUTPUT/', REGION, '/', REGION, '_',  PARAMETER_SET, '_', LIKELIHOOD_TYPE, '_denspanel.png'));
        ...
        saveas(t5, strcat('OUTPUT/', REGION, '/', REGION, '_',  PARAMETER_SET, '_', LIKELIHOOD_TYPE, '_fits.png'));
