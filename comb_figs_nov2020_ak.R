...
# NYC
load("/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/NY_Reopening_SweepLong.RData")
...
# SOUTH FLORIDA
load("/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/Sflorida_Reopening_SweepLong.RData")
...
# WASHINGTON
load("/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/Wash_Reopening_SweepLong.RData")
...
conds.nyc<-data.frame(sweepnum=rep(97, 6), alpha=rep(c(1, 5), 3), specificity=rep(c(rep(0.5, 2), rep(0.96, 2), rep(0.998, 2))),
...
ggplot(data=subset(End.nyc2, End.nyc2$testval==0|(End.nyc2$testval>0 &End.nyc2$alpha==alphaplot)), 
  ...
  geom_abline(intercept=52374, color='grey60')+
  annotate("text", x = 140000, y=52374, label=paste("\nNo testing"), angle=0, color="grey40", size=4) +
  scale_color_viridis_d(begin = 0, end = 0.9,
                        name="Specificity",
                        labels=c("50%", "96%", "99.8%")) +
  xlab("Daily testing rate") +
  ylab("Cumulative deaths") +
  lims(y=c(0, 2.75*52374))+
...
conds.sf<-data.frame(sweepnum=rep(97, 6), alpha=rep(c(1, 5), 3), specificity=rep(c(rep(0.5, 2), rep(0.96, 2), rep(0.998, 2))),
                      start.time.test=rep(248, 6), testval=rep(0, 6))
End.sflor.nt2<-merge(End.sflor.nt, conds.sf, by='sweepnum')
End.sflor2<-rbind(subset(End.sflor, !(End.sflor$testval==0) & End.sflor$start.time.test==248), End.sflor.nt2)
...
ggplot(data=subset(End.sflor2, End.sflor2$testval==0|(End.sflor2$testval>0 &End.sflor2$alpha==alphaplot)), 
  ...
  geom_line(aes(linetype=factor(start.time.test), color=factor(specificity)))+
  #scale_linetype_manual(values=c("dotted", "solid")) +
  geom_abline(intercept=9200, color='grey60')+
  annotate("text", x = testing.sflor[5], y=9200+2000, label=paste("\nNo testing"), angle=0, color="grey40", size=4) +
  scale_color_viridis_d(begin = 0, end = 0.9) +
  xlab("Daily testing rate") +
  #ylab("Cumulative deaths") +
  labs(color = "Distancing intensity", linetype ='Test performance') +
  lims(y=c(0, 2.75*9200))+
...
ggplot(data=subset(End.wash2, End.wash2$testval==0|(End.wash2$testval>0 & End.wash2$alpha==alphaplot)), 
       aes(x=testval, y=Deaths, group=interaction(specificity, start.time.test))) +
  geom_line(aes(linetype=factor(start.time.test), color=factor(specificity)))+
  geom_abline(intercept=17374, color='grey60')+
  lims(y=c(0, 2.75*17374))+
  annotate("text", x = testing.wash[7], y=17374, label=paste("\nNo testing"), angle=0, color="grey40", size=4) +
  #scale_linetype_manual(values=c("dotted", "solid"),
  #                      name = "Testing start date",
...
ggsave(plot=ggarrange(fig2a.nyc, fig2a.sflor, fig2a.wash, nrow=1), 
       file='/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/DeathsJune2021.pdf', device='pdf',
       w=12, h=4)
#### PART B
require(scales)
# USENYC pop
...
subset(model_time.nyc , model_time.nyc$alpha == alphaplot &
         model_time.nyc$start.time.test==dates.nyc[2] & model_time.nyc$specificity %in% c(0.96, 0.998)) -> model_time_plot2
model_time_plot2 %>%
  ...
  annotate("text", x = t0.nyc+dates.nyc[1], y=0.2, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.nyc+dates.nyc[2], y=0.2, label=paste("\nStart testing"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.nyc+dates.nyc[3], y=0.2, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.nyc+dates.nyc[4], y=0.2, label=paste("\nOpen schools"), angle=90, color="grey40", size=4) +
  #ggtitle("New York City") +
  theme(legend.position='none',
...
subset(model_time.sflor , model_time.sflor$alpha == alphaplot &
         model_time.sflor$start.time.test==dates.sflor[2] & model_time.sflor$specificity %in% c(0.96, 0.998)) -> model_time_plot2
model_time_plot2 %>%
  ...
  annotate("text", x = t0.sfl+dates.sflor[1], y=0.2, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.sfl+dates.sflor[2], y=0.2, label=paste("\nStart testing"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.sfl+dates.sflor[3], y=0.2, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.sfl+dates.sflor[4], y=0.2, label=paste("\nOpen schools"), angle=90, color="grey40", size=4) +
  #ggtitle("New York City") +
  theme(legend.position='none',
        #legend.position=c(0.6, 0.75),    
        legend.key = element_blank(),
...
subset(model_time.wash , model_time.wash$alpha == alphaplot &
         model_time.wash$start.time.test==dates.wash[2] & model_time.wash$specificity %in% c(0.96, 0.998)) -> model_time_plot2
model_time_plot2 %>%
  ggplot(aes(x=Date, y=FracReleased, group = sweepnum, color = factor(testval))) +
  geom_line(aes(linetype=factor(specificity))) +
  xlab("Date") +
  ...
  annotate("text", x = t0.wash+dates.wash[1], y=0.2, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.wash+dates.wash[2], y=0.2, label=paste("\nStart testing"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.wash+dates.wash[3], y=0.2, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = t0.wash+dates.wash[4], y=0.2, label=paste("\nOpen schools"), angle=90, color="grey40", size=4) +
  #ggtitle("New York City") +
  theme(#legend.position='none',
...
ggsave(fig2, file='/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/Fig2_revisAK_nolegend_align.pdf', 
...
ggplot(data=subset(model_time.nyc, model_time.nyc$start.time.test==dates.nyc[2] & model_time.nyc$alpha==5 & model_time.nyc$specificity %in% c(0.96, 0.998)), 
...
ggplot(data=subset(model_time.sflor, model_time.sflor$start.time.test==dates.sflor[2] & 
                     model_time.sflor$alpha==5 & model_time.sflor$specificity %in% c(0.96, 0.998)), 
       aes(x=Date, y=CriticalCare, group = sweepnum, color = factor(testval))) +
  geom_line(aes(linetype=factor(specificity))) +
  scale_x_date(date_breaks='4 months',labels = date_format("%b-%Y"))+
  ylab("Critical care cases") +
...
ggplot(data=subset(model_time.wash, model_time.wash$start.time.test==dates.wash[2] & model_time.wash$alpha==5 & model_time.wash$specificity %in% c(0.96, 0.998)), 
       aes(x=Date, y=CriticalCare, group = sweepnum, color = factor(testval))) +
  geom_line(aes(linetype=factor(specificity))) +
  scale_x_date(date_breaks='4 months',labels = date_format("%b-%Y"))+
  ylab("Critical care cases") +
...
ggsave(fig3, file='/Users/aliciakraay/Dropbox/COVID Shields Emory/Fig3_revis_AK.pdf', 
