...
for(i in 1:(dim(interventions.sweep)[1])){
  deathssample[[i]] = matrix(NA,length(t),numsamples)
  hospsample[[i]] = matrix(NA, length(t), numsamples)
  critsample[[i]] = matrix(NA, length(t), numsamples)
  releasedsample[[i]] = matrix(NA, length(t), numsamples)
  cisample[[i]] = matrix(NA, length(t), numsamples)
}
...
#source('/Users/aliciakraay/Dropbox/COVID Shields Emory/Shields_ODEOnly.R')
...
#save.image(file='/home/amullis/shields/lhs_nyc_test.RData')
