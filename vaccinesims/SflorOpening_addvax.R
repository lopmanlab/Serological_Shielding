#General setup
#This script estimates impact of a shielding strategy for Sflor with vaccination added
...
seir_model_shields_nopreH <- function(t, x, params) {
  ...
  prev.pos<-sum(tot.c.pos, tot.a.pos, tot.fc.pos, tot.rc.pos, tot.e.pos)/sum(tot.c, tot.a, tot.rc, tot.fc, tot.e)
  ...
  WorkContacts_10x10_Distancing[5,]<-c(WorkContacts_5x5[3,1]*FracReleased[1]*preduced, WorkContacts_5x5[3,1]*FracDistanced[1]*preduced, 
  ...
  WorkContacts_10x10_Distancing[7,]<-c(WorkContacts_5x5[4,1]*FracReleased[1]*pfull, WorkContacts_5x5[4,1]*FracDistanced[1]*pfull, 
                                       WorkContacts_5x5[4,2]*FracReleased[2]*pfull, WorkContacts_5x5[4,2]*FracDistanced[2]*pfull,
                                       WorkContacts_5x5[4,3]*FracReleased[3]*pfull, WorkContacts_5x5[4,3]*FracDistanced[3]*pfull,
                                       WorkContacts_5x5[4,4]*FracReleased[4]*pfull, WorkContacts_5x5[4,4]*FracDistanced[4]*pfull,
                                       WorkContacts_5x5[4,5]*FracReleased[5]*pfull, WorkContacts_5x5[4,5]*FracDistanced[5]*pfull)
  WorkContacts_10x10_Distancing[8,]<-WorkContacts_10x10_Distancing[7,]
  
  #Relaxed distancing work
  WorkContacts_10x10_Distancing2<-matrix(0, ncol=10, nrow=10)
  WorkContacts_10x10_Distancing2[5,]<-c(WorkContacts_5x5[3,1]*FracReleased[1]*preduced2, WorkContacts_5x5[3,1]*FracDistanced[1]*preduced2, 
                                        WorkContacts_5x5[3,2]*FracReleased[2]*preduced2, WorkContacts_5x5[3,2]*FracDistanced[2]*preduced2,
                                        WorkContacts_5x5[3,3]*FracReleased[3]*preduced2, WorkContacts_5x5[3,3]*FracDistanced[3]*preduced2,
                                        WorkContacts_5x5[3,4]*FracReleased[4]*preduced2, WorkContacts_5x5[3,4]*FracDistanced[4]*preduced2,
                                        WorkContacts_5x5[3,5]*FracReleased[5]*preduced2, WorkContacts_5x5[3,5]*FracDistanced[5]*preduced2) 
  WorkContacts_10x10_Distancing2[6,]<-WorkContacts_10x10_Distancing2[5,]
  #Full contact workers keep all of their baseline contacts
  #The distribution of those contacts is based on prevalence in the population
  WorkContacts_10x10_Distancing2[7,]<-c(WorkContacts_5x5[4,1]*FracReleased[1]*pfull, WorkContacts_5x5[4,1]*FracDistanced[1]*pfull, 
                                        WorkContacts_5x5[4,2]*FracReleased[2]*pfull, WorkContacts_5x5[4,2]*FracDistanced[2]*pfull,
                                        WorkContacts_5x5[4,3]*FracReleased[3]*pfull, WorkContacts_5x5[4,3]*FracDistanced[3]*pfull,
                                        WorkContacts_5x5[4,4]*FracReleased[4]*pfull, WorkContacts_5x5[4,4]*FracDistanced[4]*pfull,
                                        WorkContacts_5x5[4,5]*FracReleased[5]*pfull, WorkContacts_5x5[4,5]*FracDistanced[5]*pfull)
  WorkContacts_10x10_Distancing2[8,]<-WorkContacts_10x10_Distancing[7,]
  ...
  #Relaxed social distancing ohter contacts
  OtherContacts_10x10_Distancing2<-OtherContacts_10x10_Baseline*sd.other2
  ...
  if(t>=start.time.distance & t<start.time.reopen){ #Use these matrices under general social distancing without reopening 
    #or targeting
    CM<-HomeContacts_10x10 + WorkContacts_10x10_Distancing + OtherContacts_10x10_Distancing
  }
  
  #if(t>start.time.reopen & prev.pos<start.p.target){ #Use this once reopening has begun, but targeting has not yet begun
  #  CM<-HomeContacts_10x10 + WorkContacts_10x10_Distancing2 + OtherContacts_10x10_Distancing2
  #}
  
  if(t>start.time.reopen & t<school.start.time){ 
    
    #Use these matrices when p is high enough that targeting has begun but schools are not yet open
    #This could happen if it's not yet time for schools to open or if it is time to reopen, but schools
    #are all virtual 
    
    CM<-HomeContacts_10x10 + WorkContacts_10x10_TargetedDistancing + 
      OtherContacts_10x10_TargetedDistancing2}
  if(t>=school.start.time){ 
    #Use these matrices with targeting and schools reopening
    CM<-HomeContacts_10x10 + WorkContacts_10x10_TargetedDistancing + 
      OtherContacts_10x10_TargetedDistancing2+(sch.red*SchoolContacts_10x10_Baseline)
  }
  ...
  #Vaccination
  diff.E<-0; diff.FC<-0; diff.RC<-0;
  
  dosesC<-daily.vax*agestruc[1]*0 #No child vaccination
  dosesE<-daily.vax*0.4 #Give 40% of doses to elderly
  vax.e<-dosesE/sum(S.e, E.e, Ia.e, Hs.e, Hc.e, R.e)
  if(vax.e>1){vax.e<-1
  diff.E<-daily.vax*0.4-sum(S.e, E.e, Ia.e, Hs.e, Hc.e, R.e)} #Leftover doses, give to next highest risk group
  dosesFC<-daily.vax*0.4+diff.E #Give other 40% to high contact workers
  vax.fc<-dosesFC/sum(S.fc, E.fc, Ia.fc, Hs.fc, Hc.fc, R.fc)
  if(vax.fc>1){vax.fc<-1
  diff.FC<-daily.vax*0.4-sum(S.fc, E.fc, Ia.fc, Hs.fc, Hc.fc, R.fc)}
  dosesRC<-daily.vax*0.1+diff.FC #10% each to reduced contact workers and work from home workers
  vax.rc<-dosesRC/sum(S.rc, E.rc, Ia.rc, Hs.rc, Hc.rc, R.rc)
  if(vax.rc>1){vax.rc<-1
  diff.RC<-daily.vax*0.1-sum(S.rc, E.rc, Ia.rc, Hs.rc, Hc.rc, R.rc)}
  dosesA<-daily.vax*0.1 #10% each to reduced contact workers and work from home workers
  vax.a<-dosesA/sum(S.a, E.a, Ia.a, Hs.a, Hc.a, R.a)
  if(vax.a>1){vax.a<-1}
  #Model equations
  ...
  dS.a = - foi.a.gen*S.a - (1-specificity)*test.a*test.switch2*S.a-vswitch*vax.a*S.a 
  ...
  dR.a = (1-hosp_frac[2])*gamma_s*Is.a + gamma_a*Ia.a + (1-sensitivity)*test.switch2*gamma_hs*Hs.a + 
    ...
    gamma_hc*test.switch1*Hc.a*(1-crit_die[2])+test.switch1*gamma_hs*Hs.a-vswitch*vax.a*R.a  
  dS.a.pos = (1-specificity)*test.a*test.switch2*S.a - foi.a.pos*S.a.pos+vswitch*vax.a*(1-r)*S.a  
  ...
  dR.a.pos = sensitivity*test.a*test.switch2*R.a + sensitivity*test.switch2*gamma_hc*Hc.a*(1-crit_die[2]) + sensitivity*test.switch2*gamma_hs*Hs.a +
    (1-hosp_frac[2])*gamma_s*Is.a.pos + gamma_a*Ia.a.pos+vswitch*vax.a*r*S.a+vswitch*vax.a*R.a
  
  dS.rc = - foi.rc.gen*S.rc - (1-specificity)*test.rc*test.switch2*S.rc-vswitch*vax.rc*S.rc 
  ...
  dR.rc = (1-hosp_frac[2])*gamma_s*Is.rc + gamma_a*Ia.rc + (1-sensitivity)*test.switch2*gamma_hs*Hs.rc + 
    ...
    gamma_hc*test.switch1*Hc.rc*(1-crit_die[2])+test.switch1*gamma_hs*Hs.rc-vswitch*vax.rc*R.rc 
  dS.rc.pos = (1-specificity)*test.rc*test.switch2*S.rc - foi.rc.pos*S.rc.pos+vswitch*vax.rc*(1-r)*S.rc 
  ...
  dR.rc.pos = sensitivity*test.rc*test.switch2*R.rc + sensitivity*test.switch2*gamma_hc*Hc.rc*(1-crit_die[2]) + sensitivity*test.switch2*gamma_hs*Hs.rc +
    (1-hosp_frac[2])*gamma_s*Is.rc.pos + gamma_a*Ia.rc.pos+vswitch*vax.rc*(r*S.rc+R.rc)  
  
  dS.fc = - foi.fc.gen*S.fc - (1-specificity)*test.fc*test.switch2*S.fc-vswitch*vax.fc*S.fc 
  ...
  dR.fc = (1-hosp_frac[2])*gamma_s*Is.fc + gamma_a*Ia.fc + (1-sensitivity)*test.switch2*gamma_hs*Hs.fc + 
    ...
    gamma_hc*test.switch1*Hc.fc*(1-crit_die[2])+test.switch1*gamma_hs*Hs.fc-vswitch*vax.fc*R.fc 
  dS.fc.pos = (1-specificity)*test.fc*test.switch2*S.fc - foi.fc.pos*S.fc.pos+vswitch*vax.fc*(1-r)*S.fc 
  ...
  dR.fc.pos = sensitivity*test.fc*test.switch2*R.fc + sensitivity*test.switch2*gamma_hc*Hc.fc*(1-crit_die[2]) + sensitivity*test.switch2*gamma_hs*Hs.fc +
    (1-hosp_frac[2])*gamma_s*Is.fc.pos + gamma_a*Ia.fc.pos+vswitch*vax.fc*(R.fc+r*S.fc)
  
  dS.e = - foi.e.gen*S.e - (1-specificity)*test.e*test.switch2*S.e-vswitch*vax.e*S.e 
  ...
  dR.e = (1-hosp_frac[3])*gamma_s*Is.e + gamma_a*Ia.e + (1-sensitivity)*test.switch2*gamma_hs*Hs.e + 
    ...
    gamma_hc*test.switch1*Hc.e*(1-crit_die[3])+test.switch1*gamma_hs*Hs.e-vswitch*vax.e*R.e 
  dS.e.pos = (1-specificity)*test.e*test.switch2*S.e - foi.e.pos*S.e.pos+vswitch*vax.e*(1-r)*S.e 
  ...
  dR.e.pos = sensitivity*test.e*test.switch2*R.e + sensitivity*gamma_hc*test.switch2*Hc.e*(1-crit_die[3]) + sensitivity*gamma_hs*test.switch2*Hs.e +
    (1-hosp_frac[3])*gamma_s*Is.e.pos + gamma_a*Ia.e.pos+vswitch*vax.e*(R.e+r*S.e)
  
  dH.pos=gamma_s*Is.c.pos*hosp_frac[1]+gamma_s*Is.a.pos*hosp_frac[2]+gamma_s*Is.e.pos*hosp_frac[3]
  ...
  res = c(dS.c, dE.c, dIs.c, dIa.c, dHs.c, dHc.c, dD.c, dR.c, dS.c.pos, dE.c.pos, dIs.c.pos, dIa.c.pos, dR.c.pos,
          ...
          dS.e, dE.e, dIs.e, dIa.e, dHs.e, dHc.e, dD.e, dR.e, dS.e.pos, dE.e.pos, dIs.e.pos, dIa.e.pos, dR.e.pos,
          dH.pos)
...
# from AgeStructure_byLocation.xlsx
agestruc=c(0.23, 0.60, 0.18)
...
asymp.red=0.55 #relative infectiousness of asymptomatic infections compared to symptomatic infections
...
testvec<-rep(0, 475) #Change this for time series length
times <- seq(0, 474, length = 475) #Change this for time series length
#daily.vax<-0.005*N
daily.vax<-0
r<-0.95
...
#Initial conditions are from Conan - absolute file path
initcond<-read.csv('/Users/aliciakraay/Dropbox/COVIDShieldsEmory/MetroFits/2021-06-17_MLEs.csv')
initcond.sflor<-initcond[2,]
# base model
start = c(S.c = initcond.sflor$S_c, E.c =initcond.sflor$E_c, Is.c = initcond.sflor$Isym_c, Ia.c = initcond.sflor$Iasym_c, 
          Hs.c = initcond.sflor$Hsub_c, Hc.c =initcond.sflor$Hcri_c, D.c = initcond.sflor$D_c, R.c = initcond.sflor$R_c, 
          S.c.pos = 0, E.c.pos = 0, Is.c.pos = 0, Ia.c.pos = 0, R.c.pos =0, 
          
          S.a = initcond.sflor$S_a, E.a = initcond.sflor$E_a, Is.a = initcond.sflor$Isym_a, Ia.a = initcond.sflor$Iasym_a, 
          Hs.a = initcond.sflor$Hsub_a, Hc.a = initcond.sflor$Hcri_a, D.a = initcond.sflor$D_a, R.a = initcond.sflor$R_a, 
          S.a.pos = 0, E.a.pos = 0, Is.a.pos = 0, Ia.a.pos = 0, R.a.pos =0,
          
          S.rc = initcond.sflor$S_rc, E.rc = initcond.sflor$E_rc, Is.rc = initcond.sflor$Isym_rc, Ia.rc = initcond.sflor$Iasym_rc, 
          Hs.rc = initcond.sflor$Hsub_rc, Hc.rc = initcond.sflor$Hcri_rc, D.rc = initcond.sflor$D_rc, R.rc = initcond.sflor$R_rc, 
          S.rc.pos = 0, E.rc.pos = 0, Is.rc.pos = 0, Ia.rc.pos = 0, R.rc.pos =0,
          
          S.fc = initcond.sflor$S_fc, E.fc = initcond.sflor$E_fc, Is.fc = initcond.sflor$Isym_fc, Ia.fc = initcond.sflor$Iasym_fc, 
          Hs.fc = initcond.sflor$Hsub_fc, Hc.fc = initcond.sflor$Hcri_fc, D.fc = initcond.sflor$D_fc, R.fc = initcond.sflor$R_fc, 
          S.fc.pos = 0, E.fc.pos = 0, Is.fc.pos = 0, Ia.fc.pos = 0, R.fc.pos =0,
          
          S.e = initcond.sflor$S_e, E.e = initcond.sflor$E_e, Is.e = initcond.sflor$Isym_e, Ia.e = initcond.sflor$Iasym_e, 
          Hs.e = initcond.sflor$Hsub_e, Hc.e= initcond.sflor$Hcri_e, D.e = initcond.sflor$D_e, R.e = initcond.sflor$R_e, 
          S.e.pos = 0, E.e.pos = 0, Is.e.pos = 0, Ia.e.pos = 0, R.e.pos =0,
          H.pos = 0)


p<-as.numeric(initcond.sflor$symptomatic_fraction)
q<-as.numeric(initcond.sflor$q)
sd.other<-as.numeric(initcond.sflor$socialDistancing_other)
preduced<-as.numeric(initcond.sflor$p_reduced)
c<-as.numeric(initcond.sflor$c)
...
testvec<-rep(0, 475)
times <- seq(0, 474, length = 475)

# START DATE: Feb 27, 2020
# START DISTANCING: March 17, 2020
# REOPEN: May 20, 2020
# SCHOOLS OPEN: September 1, 2020

#Intervention timing based on local policies
start.time.distance <- 19 
start.time.reopen <- as.numeric(as.Date('05/20/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')) 
school.start.time <- as.numeric(as.Date('9/01/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y'))  #


# CHANGE
#can change when in the outbreak testing becomes available
#start.time.test<-as.numeric(as.Date('11/01/2020', format='%m/%d/%Y')-as.Date('02/20/2020', format='%m/%d/%Y'))
start.time.test<-start.time.reopen
start.time.vax<-as.numeric(c(as.Date('1/01/2021', format='%m/%d/%Y')-as.Date('02/20/2020', format='%m/%d/%Y')))
...
#alpha2<-c(1, 5, 10)
alpha2<-c(1, 5)
specificity<-c(0.5, 0.90, 0.998)
testing<-c(0, N/(365*100), N/(365*10), N/(365*5), N/365, N/180, N/90, N/60, N/30)
#can change when in the outbreak testing becomes available
#here we have 1) early (with SAH order lifting) and 2) late option (11/01/2020)
start.time.test<-as.numeric(c(start.time.reopen, as.Date('11/01/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')))
start.time.vax<-as.numeric(c(as.Date('1/01/2021', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')))

sch.red<-0.5

sweep<-expand.grid(testval=testing, alpha=alpha2,
                   specificity=specificity, start.time.test=start.time.test)
sweep.test<-subset(sweep, sweep$testval>0)
sweep.notest<-subset(sweep, sweep$testval==0)
sweep<-rbind(sweep.test, sweep.notest[1,])
...
for(i in 1:dim(sweep)[1]){
  data.summary[[i]]<-data.frame(nrow=475, ncol=10)
  data.full[[i]]<-data.frame(nrow=475, ncol=10)
}

start.time<-Sys.time()
for(i in 1:dim(sweep)[1]){
  #for(i in 1:5){
  start.time.test<-sweep$start.time.test[i]
  start.p.target<-sweep$start.target[i]
  ...
  if(daily.tests==0){
    tswitch1.dat<-data.frame(times=times, test.switch1=c(rep(1, 475)))
    tswitch2.dat<-data.frame(times=times, test.switch2=c(rep(0, 475)))
  }
  if(daily.tests>0){
  ...
  vswitch.dat<-data.frame(times=times, doses=c(rep(0, start.time.vax+1),
                                               rep(1, length(times)-(start.time.vax+1))))
  ...
  vstart<-approxfun(vswitch.dat$times, vswitch.dat$doses, rule=2)
...
Total.time
save.image('/Users/aliciakraay/Dropbox/COVIDShieldsEmory/MetroFits/Sflor_Reopening_SweepLong_NoVax_July2021.RData')
#save.image('/Users/kristinnelson/Box Sync/covid-19/Shielding/Results/Sflorida_Reopening_Sweep2.RData')

#save.image('/Users/aliciakraay/Dropbox/COVID Shields Emory/MetroFits/NY_Reopening_CredInt.RData')
require(ggpubr)

schooltime <- school.start.time
testtime <- start.time.test
reopentime <- start.time.reopen
distancetime <- start.time.distance


dates <- c(distancetime, testtime, reopentime, schooltime)
End<-subset(model_time, model_time$time==467) #Can see this here

write.csv(End, '/Users/aliciakraay/Dropbox/COVIDShieldsEmory/Sflor_novaccination.csv')
...
ggplot(data=subset(End, End$alpha==alphaplot), 
  ...
  scale_x_log10(breaks=c(0, N/(365*100), N/(365*10), N/365, N/30),
  ...
  theme(legend.position='none',
        ...
        axis.ticks.length=unit(.25, "cm"))+labs(color='Specificity', linetype='Start target') -> fig1a1; fig1a1

alphaplot <- 5
ggplot(data=subset(End, End$alpha==alphaplot & End$start.time.test==248), 
       aes(x=testval, y=Deaths, group=specificity))+
  geom_line(aes(color=factor(specificity)))+
  scale_linetype_manual(values=c("dotted", "solid")) +
  scale_color_viridis_d(begin = 0, end = 0.9) +
  xlab("Daily testing rate") +
  ylab("Cumulative deaths") +
  labs(color = "Distancing intensity", linetype ='Test performance') +
  scale_x_log10(breaks=c(0, 120, 1200, 12000, 140000),
                labels=c('0', '1%/yr', '10%/yr', 'Yearly', 'Monthly')) +
  theme(legend.position=c(0.3, 0.7),
        ...
        legend.box='horizontal')+labs(color='Specificity', linetype='Start target') -> fig1a5; fig1a5
...
NoTest.rep$alpha<-rep(Conds$alpha, each=366)
NoTest.rep$specificity<-rep(Conds$specificity, each=366)
NoTest.rep$start.time.test<-rep(Conds$start.time.test, each=366)
NoTest.rep$sweepnum<-rep(c(101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112), each=366)
model_time_tested<-subset(model_time, model_time$testval>0)
model_time2<-rbind(model_time_tested, NoTest.rep)

model_time3 <-subset(model_time2, !(model_time2$testval %in% c(N/90, N/60, N/180, N/(365*5))))

###Second plot set: fraction seropositive###
alphaplot<-5
subset(model_time3 , model_time3$alpha == alphaplot &
         model_time3$start.time.test == as.numeric(as.Date('11/01/2020', format='%m/%d/%Y')-as.Date('02/27/2020', format='%m/%d/%Y')) & model_time3$specificity %in% c(0.96, 0.998)) -> model_time_plot2
model_time_plot2 %>%
  ...
  coord_cartesian(xlim=c(0, 474), ylim=c(0, 1)) +
  geom_vline(xintercept = dates[c(1,3,4)], color = "grey60") +
  annotate("text", x = dates[1], y=0.2, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[3], y=0.2, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[4]+10, y=0.18, label=paste("\nOpen schools"), angle=90, color="grey40", size=4) +
  ggtitle("NYC, 5:1 Shielding") +
  theme(legend.position='none',
...
subset(model_time , model_time$alpha == alphaplot &
         model_time$start.target==0.05 & model_time$specificity %in% c(0.96, 0.998)) -> model_time_plot
model_time_plot %>%
  ggplot(aes(x=time, y=Released/N, group = sweepnum, color = factor(testval))) +
  geom_line(aes(linetype=factor(specificity))) +
  xlab("Days since Jan 15, 2020") +
  ...
  scale_color_viridis_d(option="plasma", begin = 0.1, end = 0.9, direction = -1,
                        ...
                        labels=c("No testing", "1% per year", "10% per year",  "Yearly", "Monthly")) +
  #labs(color = "Per capita testing rate") +
  coord_cartesian(xlim=c(0, 365), ylim=c(0, 0.5)) +
  geom_vline(xintercept = dates[c(1,3,4)], color = "grey60") +
  annotate("text", x = dates[1], y=0.2, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[3], y=0.2, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[4]+10, y=0.18, label=paste("\nOpen schools if \n seropositives > 5%"), angle=90, color="grey40", size=4) +
  ggtitle("NYC, 5:1 Shielding") +
  theme(#legend.position='none',
    ...
    axis.line.y = element_line(colour = 'black', size=0.25, linetype='solid'),
    axis.ticks.length=unit(.25, "cm"))+
  labs(color='Per capita testing rate', linetype='Specificity')-> fig2a1; fig2a1


subset(model_time , model_time$alpha == alphaplot &
         model_time$start.target==0.1 & model_time$specificity %in% c(0.96, 0.998)) -> model_time_plot
model_time_plot %>%
  ggplot(aes(x=time, y=Released/N, group = sweepnum, color = factor(testval))) +
  geom_line(aes(linetype=factor(specificity))) +
  xlab("Days since Jan 15, 2020") +
  ...
  annotate("text", x = dates[1], y=0.2, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[3], y=0.2, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[4]+10, y=0.18, label=paste("\nOpen schools if \n seropositives > 10%"), angle=90, color="grey40", size=4) +
  ggtitle("NYC, 5:1 Shielding") +
  theme(#legend.position='none',
    legend.position=c(0.4, 0.7),    
    legend.key = element_blank(),
    ...
    axis.line.y = element_line(colour = 'black', size=0.25, linetype='solid'),
    axis.ticks.length=unit(.25, "cm"))+
  labs(color='Per capita testing rate', linetype='Specificity') -> fig2a2; fig2a2
...
model_time_plot %>%
  ...
  #labs(color = "Per capita testing rate") +
  geom_vline(xintercept = dates[c(1,3,4)], color = "grey60") +
  annotate("text", x = dates[1], y=500, label=paste("\nGeneral distancing"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[3], y=500, label=paste("\nSAH order lifted"), angle=90, color="grey40", size=4) +
  annotate("text", x = dates[4]+10, y=500, label=paste("\nOpen schools if \n seropositives > 10%"), angle=90, color="grey40", size=4) +
  ggtitle("NYC, 5:1 Shielding") +
  theme(#legend.position='none',
    legend.position=c(0.4, 0.7),    
    legend.key = element_blank(),
...
subset(End, End$Hospital.pos>1000 & End$specificity==0.5)
LowSpec<-subset(model_time, model_time$specificity==0.5 & model_time$alpha==5)
LowSpec.flag<-subset(LowSpec, LowSpec$Hospital.pos>=1000)
RecognizeImm<-ddply(LowSpec.flag, .(sweepnum), summarize, TFlag=min(time))
Check<-subset(LowSpec, LowSpec$sweepnum %in% unique(RecognizeImm$sweepnum))
Check5<-subset(Check, Check$FracReleased>=0.05)
Check10<-subset(Check, Check$FracReleased>=0.10)
Release5<-ddply(Check5, .(sweepnum), summarize, T5=min(time))
Release10<-ddply(Check10, .(sweepnum), summarize, T10=min(time))

Recog5<-merge(RecognizeImm, Release5, by='sweepnum')
Recog5to10<-merge(Recog5, Release10, by='sweepnum')
Check2<-merge(Recog5to10, sweep, by='sweepnum')
Check2$reopen.t<-NA
Check2$reopen.t[which(Check2$start.target==0.05)]<-Check2$TFlag-Check2$T5
Check2$reopen.t[which(Check2$start.target==0.1)]<-Check2$TFlag-Check2$T10
