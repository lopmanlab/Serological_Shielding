...
null.model$week<-c(0, rep(c(1:52), each=7), 53)
WeekDeaths<-ddply(null.model, .(week), summarize, DeathsWk=sum(Deaths))
...
tolerance<-1e-20 #tolerance for the solver, not sure this is needed here
DatVec<-WeekDeaths[c(1:17),2]
lik<-dnbinom(NYC$wkdeaths[8:24],psi, mu=rho*Deaths+tolerance, log=TRUE)

NYC$DeathsModeled<-c(0, 0, 0, 0, 0, 0, 0, DatVec)
