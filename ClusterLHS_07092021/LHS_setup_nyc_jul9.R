...
#hist(paramsample[,4]); hist(paramsample[,5]); hist(paramsample[,6])
...
#Fitted parameters-simulating at the MLE for these--this is for NYC
...
dat<-foreach(i=1:numsamples, .combine=c, .packages=c('lhs', 'deSolve', 'ggplot2', 'gridExtra', 'plyr')) %dopar% {
  tryCatch(expr={
    ...
    for(j in 1:nrow(interventions.sweep)){
      ...
      if(interventions.sweep$testval[j]>0){daily.tests<-9437195/interventions.sweep$testval[j]}
      ...
      test<-ddply(model_out, .(time), summarize, deaths=sum(D.c, D.a, D.e, D.rc, D.fc),
                  ...
                  ci=(9437195-sum(S.c, S.c.pos, S.a, S.a.pos, S.rc, S.rc.pos, S.fc, S.fc.pos, S.e, S.e.pos))/9437195)
