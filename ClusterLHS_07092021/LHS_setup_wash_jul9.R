...
#Parameters to use for LHS:
#LOS for ICU (gamma_hc): 6-20d
#LOS for non-ICU (gamma_hs): 3-9d
#Recovery for asymptomatic (gamma_a): 3-8d
#Recovery for symptomatic, not hospitalized (gamma_s): 1-10d (possibly up to 10?)
#Incubation period (gamma_e): 3-12d 
#Transmissibility for asymptomatic infections (asy): 25-100%

set.seed(8)
numsamples = 3000 #Doing 500 per parameter varied for now
paramsample = randomLHS(numsamples,6)


# these are containers that we'll fill in--lists with outer index for intervention id 
#and the inner for parameter set (from LHS)
#Here is the intervention sweep
interventions.sweep<-data.frame(testval=c(0, 30, 30, 30, 365, 30, 365), 
                                specificity=c(0.5, 0.96, 0.96, 0.998, 0.998, 0.998, 0.998),
                                #start.time.test=c(255, 255, 255, 255, 255, 108, 108),
                                test.timing=c('Nov1', 'Nov1', 'Nov1', 'Nov1', 'Nov1', 'SAH', 'SAH'),
                                alpha=c(5, 1, 5, 5, 5, 5, 5), 
...
# set upper and lower bounds for the parameters - order is gamma_hc, gamma_hs, gamma_a, gamma_s, gamma_e, asy 
paramlower = c(6,3,3,1,3,0.25)
paramupper = c(20,9,8,10,12,1)

###Get parameters set up for the model####
#Matrix parameters-prem et al
...
#Expand the 3x3 matrix to a 5x5 based on the fraction of the population in each of the worker subgroups
...
#Fixed params-not varied by intervention or LHS
sensitivity<-1.0
sch.red<-0.5
...
#times <-seq(0, 250, length=251)

#Fitted parameters-simulating at the MLE for these--this is for Washington
initcond <- read.csv("2021-06-17_MLEs.csv")
#initcond<-read.csv('/home/amullis/shields/2021-06-17_MLEs.csv')
initcond<-initcond[3,]
...
####Location specific parameters, here for NYC####
#Order is: nyc, sfl, wash
N=4064835
start.time.distance<-32
start.time.reopen<-108
school.start.time<-231

agestruc.c<-0.24
agestruc.a<-0.63
agestruc.e<-0.14
agestruc<-c(agestruc.c, agestruc.a, agestruc.e)
nov1.numeric<-262

#source('/Users/aliciakraay/Dropbox/COVIDShieldsEmory/Shields_ODEOnly.R')
#Pasting the ODE here
#Shields ODE
...
dat<-foreach(i=1:numsamples, .combine=c, .packages=c('lhs', 'deSolve', 'ggplot2', 'gridExtra', 'plyr')) %dopar% {
  tryCatch(expr={
    ...
    for(j in 1:nrow(interventions.sweep)){
      ...
      if(interventions.sweep$testval[j]>0){daily.tests<-4064835/interventions.sweep$testval[j]}
      ...
      test<-ddply(model_out, .(time), summarize, deaths=sum(D.c, D.a, D.e, D.rc, D.fc),
                  ...
                  ci=(4064835-sum(S.c, S.c.pos, S.a, S.a.pos, S.rc, S.rc.pos, S.fc, S.fc.pos, S.e, S.e.pos))/4064835)
...
save.image(file='LHS_wash.RData')
